...
Moat employs a **"Fake Server" Integration Testing** strategy for its media services.
...
- **Lucene Query Validation**: Mock handlers parse and evaluate complex Lucene queries generated by the services against a "Fake Database" in memory. This ensures that filters (Artist ID, Year Ranges, Fuzzy logic) are correctly translated and handled.
- **Resilience**: This strategy makes tests resilient to internal refactoring. As long as the service interacts with the API correctly and returns valid data, the tests pass.
- **Error Simulation**: MSW allows granular simulation of API failures (503s, 500s, 404s) and rate-limiting scenarios to verify error handling logic.
