...
import { Item, TierId, TierListState, TierUpdate } from '@/features/board/types';
...
const boardSlice = createSlice({
  ...
  reducers: {
    ...
    deleteTier: (state, action: PayloadAction<{ id: string | TierId }>) =>
      handleDeleteTier(state, action),
    updateTier: (state, action: PayloadAction<{ id: string | TierId; updates: TierUpdate }>) =>
      handleUpdateTier(state, action),
    reorderTiers: (
      state,
      action: PayloadAction<{ activeId: string | TierId; overId: string | TierId }>,
    ) => handleReorderTiers(state, action),
    randomizeColors: handleRandomizeColors,

    // --- Item Actions ---
    moveItem: (state, action: PayloadAction<MoveItemPayload>) => handleMoveItem(state, action),
    removeItem: (state, action: PayloadAction<{ tierId: string | TierId; itemId: string }>) =>
      handleRemoveItem(state, action),
    updateItem: (state, action: PayloadAction<{ itemId: string; updates: ItemUpdate }>) =>
    ...
    updateTitle: (state, action: PayloadAction<{ title: string }>) =>
      handleGlobalState.updateTitle(state, action),
