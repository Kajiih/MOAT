 ...
 */
const INITIAL_REGISTRY: Record<string, LegacyItem> = {};
...
interface MediaRegistryContextType {
  ...
  registerItems: (items: LegacyItem[]) => void;
  /** Registration of a single item. */
  registerItem: (item: LegacyItem) => void;
  /** Retrieval of an item by its globally unique ID. */
  getItem: <T extends LegacyItem>(id: string) => T | undefined;
 ...
 */
function shouldUpdateItem(existing: LegacyItem, newItem: LegacyItem): boolean {
 ...
 */
export function ItemRegistryProvider({ children }: { children: React.ReactNode }) {
  const [registry, setRegistry] = usePersistentState<Record<string, LegacyItem>>(
    'moat-media-registry',
    INITIAL_REGISTRY,
  );
   ...
   */
  const pruneRegistry = useCallback((current: Record<string, LegacyItem>) => {
  ...
  const mergeItems = useCallback(
    (prev: Record<string, LegacyItem>, items: LegacyItem[]) => {
  ...
  const getItem = useCallback(
    <T extends LegacyItem>(id: string): T | undefined => {
 ...
 * @throws {Error} if used outside of a ItemRegistryProvider.
 */
export function useItemRegistry() {
  const context = useContext(MediaRegistryContext);
  if (!context) {
    throw new Error('useItemRegistry must be used within a ItemRegistryProvider');
  }
  return context;
}
