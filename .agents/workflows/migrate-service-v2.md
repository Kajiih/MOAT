...
3. **Scaffold V2 Provider**: Create a new file in `lib/services/`.
...
7. **Register**: Import the provider singleton and add it to the `providers` array in `lib/database/providers.ts`.
8. **Test**: Use the `SearchPanel` to verify the service is functional.
