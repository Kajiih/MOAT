 ...
 * 
 * @param props - Raw state and hook outputs.
...
export function useTierListNamespaces({
   ...
   */
  const updateMediaItem = useCallback((itemId: string, updates: Partial<MediaItem>, registerItem: (item: MediaItem) => void) => {
  ...
  }), [structureRaw, handleUpdateTitle, updateMediaItem, removeItemFromTier, utilsRaw.handleLocate, ioRaw]);
