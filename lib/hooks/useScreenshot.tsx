...
export function useScreenshot(fileName: string = 'tierlist.png') {
  ...
  const takeScreenshot = useCallback(async (state: TierListState, headerColors: string[]) => {
    ...
    const uniqueUrls = Array.from(new Set(
        Object.values(state.items).flat().flatMap(item => item.imageUrl ? [item.imageUrl] : [])
    ));
    ...
    console.log(`Screenshot Engine: Successfully resolved ${resolvedCount}/${uniqueUrls.length} images.`);
